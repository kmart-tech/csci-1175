...
Exercise 28_05 - add method getPath
 */
import java.util.*;
...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getPath(int u, int v) {
		SearchTree bfsTree = bfs(u);
		LinkedList<Integer> pathList = new LinkedList<Integer>();
		int currentParent = v;
		while (currentParent != -1) {
			pathList.addFirst(currentParent);
			currentParent = bfsTree.getParent(currentParent);
		}
		if (pathList.getFirst() == u) return pathList;
		else return null;
	}
