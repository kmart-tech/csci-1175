...
Exercise 28_3 - Implement nonrecursive DFS using a stack
 */
import java.util.*;
...
public class UnweightedGraph<V> implements Graph<V> {
	...
	*/
	public SearchTree dfsNonRecurs(int v) {
		...
		// add v to the isVisited, searchOrder, and stack
		isVisited[v] = true;
		searchOrder.add(v);
		stack.push(v);
		...
		while (!stack.empty()) {
			...
			boolean hadUnvisitedNeighbor = false;
			for (Edge e: neighbors.get(x)) {
				if (!isVisited[e.v]) {
					...
					isVisited[e.v] = true;
					searchOrder.add(e.v);
					hadUnvisitedNeighbor = true;
					break;
				}
			}
			// x's neighbors are already visited, pop x off the top of the stack
			if (!hadUnvisitedNeighbor) stack.pop();
		}
